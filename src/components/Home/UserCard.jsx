    ...
    <div class="flex flex-col-3 justify-between w-11/12 lg:w-3/6 h-56 opacity-100 mt-2 mb-2 lg:mb-2 bg-gray-100 rounded-lg border shadow-md hover:bg-yellow-500  hover:opacity-100">
      ...
      <div className="flex flex-col mt-2 ml-2">
        <Link to={`/users/${_id}`}>
          ...
          <h5 class=" font-bold text-xl lg:text-2xl ml-2 tracking-tight text-gray-900 dark:text-white">              {username?.length > 15 ? `${username.slice(0, 10)}...` : username}
            </h5>
          <h5 class=" font-semibold text-sm lg:text-xl ml-2 tracking-tight text-gray-900 dark:text-white">  
            ...
            {donations?.length > 0 ? (
              <span className="font-bold flex justify-center text-sm lg:text-xl text-gnormalray-700 dark:text-gray-400">
        ...
        <div className=" flex items-center w-50 h-56 lg:w-56 lg:h-56 justify-center">
          {pettit?.length === 1 ? (
            <img
              className=" h-24 w-24 lg:h-30 lg:w-30 rounded-full shadow-lg"
              src={pettit[0].image}
              alt="ProfilePicture"
            />
          ) : pettit?.length > 1 ? (
            <Carousel indicators={false} leftControl=" " rightControl=" ">
              {pettit?.map((pet) => (
                <img
                  key={pet._id}
                  className="w-24 h-24 lg:h-30 lg:w-30   rounded-full shadow-lg"
                  src={pet.image}
                  alt="ProfilePicture"
                />
              ))}
      ...
      <div className="flex flex-col justify-between h-10 w-10 m-1 lg:m-4">
        ...
        >
          <button
            onClick={() => chat()}
            className="flex justify-end m-1 mt-40 lg:mt-28"
          >
            <img
              src={chatt}
              alt=""
